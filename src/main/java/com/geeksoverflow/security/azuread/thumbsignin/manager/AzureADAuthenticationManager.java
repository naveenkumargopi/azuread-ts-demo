package com.geeksoverflow.security.azuread.thumbsignin.manager;
...
import com.geeksoverflow.security.azuread.thumbsignin.exception.AzureADAttributesValidationException;
import com.geeksoverflow.security.azuread.thumbsignin.exception.AzureADAuthenticationException;
...
import com.geeksoverflow.security.azuread.thumbsignin.service.RegisterUserDetailsService;
...
public class AzureADAuthenticationManager {
    ...
    private static final String DEV_SERVER = "52.38.227.15";
    ...
    public AzureADAuthenticationManager(AzureADClientProps azureADProps) {
        ...
        this.uselocalDB = azureADProps.isUselocalDB();
    }
    ...
    public Authentication processAuthenticationData(HttpServletRequest httpRequest, String currentUri, String fullUrl) {
        ...
        if (AuthHelper.isAuthenticationSuccessful(authResponse)) {
            ...
            try {
                validateNonce(stateData, getClaimValueFromIdToken(authResult.getIdToken(), "nonce"));
            } catch (Exception e) {
                new AzureADAttributesValidationException(e.getMessage());
            }
            ...
            if (uselocalDB) {
            	UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = new UsernamePasswordAuthenticationToken(authResult.getUserInfo().getUniqueId(), authResult.getUserInfo().getGivenName());
            	try {
                    authenticate = authenticationManager.authenticate(usernamePasswordAuthenticationToken);
                } catch (AuthenticationException e) {
                    registerUserDetailsService.register(authResult.getUserInfo());
                    authenticate = authenticationManager.authenticate(usernamePasswordAuthenticationToken);
                }
            }
            else {
            	authenticate = new UsernamePasswordAuthenticationToken(authResult.getUserInfo().getUniqueId(), authResult.getUserInfo().getGivenName());
            }
    ...
    public void sendAuthRedirect(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
        ...
        if (!currentUri.contains(LOCALHOST) && !currentUri.contains(DEV_SERVER) && !currentUri.contains(HTTPS)) {
